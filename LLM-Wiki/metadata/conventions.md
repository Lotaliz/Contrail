...
- 行内公式使用单美元符号，例如 `$z=(r,k,d,m)$`。
- 独立公式使用单独成行的双美元符号，并在公式块前后保留空行：
...
- 不使用 `\(...\)` 或 `\[...\]` 作为数学定界符；当前 Markdown 预览器不稳定支持这两种写法。
- Markdown 表格内只使用行内公式；较长公式移到表格外的独立公式块，避免渲染和列分隔冲突。
- 从论文 LaTeX、网页或 PDF 转写公式时，应转换定界符，但保留公式内部的 LaTeX 命令和符号含义。
